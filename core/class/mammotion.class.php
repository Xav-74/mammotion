...
class mammotion extends eqLogic {
    ...
    public function postSave()
	{
		...
		if ($deviceType == 'pool') {
			...
			$order++;
			$this->createCmd('wifi_rssi', 'Signal Wifi', $order, 'info', 'numeric', 1, 1);
			$order++;
			$this->createCmd('ble_rssi', 'Signal Bluetooth', $order, 'info', 'numeric', 1, 1);
			$order++;
		...
		if ($deviceType == 'mower') {
			...
			$this->createCmd('work_mode', 'Statut', $order, 'info', 'string');
			$order++;
			$this->createCmd('speed', 'Vitesse', $order, 'info', 'numeric', 1, 1);
			$order++;
			$this->createCmd('charging', 'En charge', $order, 'info', 'binary');
			$order++;
			...
			$order++;
			$this->createCmd('work_area', 'Surface tondue', $order, 'info', 'numeric', 1, 1);
			$order++;
			$this->createCmd('current_area', 'Zone courante', $order, 'info', 'string');
			$order++;
			$this->createCmd('left_time', 'Temps restant', $order, 'info', 'numeric', 1, 1);
			$order++;
			$this->createCmd('elapsed_time', 'Temps écoulé', $order, 'info', 'numeric', 1, 1);
			$order++;
			$this->createCmd('blade_height', 'Hauteur de lame', $order, 'info', 'numeric', 1, 1);
			$order++;
			...
			$this->createCmd('ble_rssi', 'Signal Bluetooth', $order, 'info', 'numeric', 1, 1);
			$order++;
			$this->createCmd('mnet_rssi', 'Signal cellulaire', $order, 'info', 'numeric', 1, 1);
			$order++;
			$this->createCmd('blade_used_time', 'Temps d\'utilisation des lames', $order, 'info', 'numeric', 1, 1);
			$order++;
			$this->createCmd('blade_used_warn_time', 'Seuil d\'usure des lames', $order, 'info', 'numeric');
			$order++;
			$this->createCmd('blade_used_left_time', 'Temps restant d\'utilisation des lames', $order, 'info', 'numeric', 1, 1);
			$order++;
			$this->createCmd('total_mileage', 'Distance totale', $order, 'info', 'numeric', 1, 1);
			$order++;
			$this->createCmd('total_work_time', 'Temps de travail total', $order, 'info', 'numeric', 1, 1);
			$order++;
			$this->createCmd('bat_cycles', 'Cycles batterie', $order, 'info', 'numeric', 1, 1);
			$order++;
			$this->createCmd('firmware', 'Firmware', $order, 'info', 'string');
			$order++;
